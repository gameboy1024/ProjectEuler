...
'''
  ...
  Answer: 376 Completed on Fri, 7 Nov 2014, 16:09
...
class PokerHand(object):
  ...
  def __init__(self, hands_string):
    self.cards = []
    for i in xrange(0, 13, 3):
      self.cards.append(hands_string[i : i + 2])
    self.sorted_cards = [i for i in sorted(
        self.cards, key=lambda card: get_key(card), reverse=True)]
  ...
  # The following functions with name of a poker hand returns a str, for the 
  # score, None for not available(not this hand), else for scores.
  # We have 10 kinds of hands and each has many possibilites, then we use a 
  # letter in first to indicate the kinds, A for RoyalFlush, B for Straight
  # Flush, etc. Then we add the values, if applicable for comparison, finally 
  # this value can be used to compare with python native < or >.
  
  def royal_flush(self):
    # Must be of same suit.
    suit = self.sorted_cards[0][1]
    for card in self.sorted_cards:
      if card[1] != suit:
        return None
    royal_flush_suit = ['A', 'K', 'Q', 'J', 'T']
    if self.sorted_cards == royal_flush_suit:
      return 'A'
    return None
  ...
  def straight_flush(self):
    ...
    for card in self.sorted_cards:
      if card[1] != suit:
        return None
    order = 'AKQJT98765432'
    if ''.join(self.sorted_cards) in order:
      return 'B' + self.sorted_cards[0][0]
    return None
      
  def four_of_a_kind(self):
    flag = True
    for i in xrange(0, 4):
      if self.sorted_cards[i][0] != self.sorted_cards[i + 1][0]:
        flag = False
        break
    if flag == True:
      return 'C' + self.sorted_cards[0][0] + self.sorted_cards[4][0]
    ...
    for i in xrange(1, 5):
      if self.sorted_cards[i][0] != self.sorted_cards[i + 1][0]:
        ...
        break
    if flag == True:
      return 'C' + self.sorted_cards[1][0] + self.sorted_cards[0][0]
    return None
    
  def full_house(self):
    if (self.sorted_cards[0][0] == self.sorted_cards[1][0] and
        self.sorted_cards[2][0] == self.sorted_cards[3][0] and 
        self.sorted_cards[3][0] == self.sorted_cards[4][0]):
      return 'D' + self.sorted_cards[2][0] + self.sorted_cards[0][0]
    if (self.sorted_cards[0][0] == self.sorted_cards[1][0] and
        self.sorted_cards[1][0] == self.sorted_cards[2][0] and 
        self.sorted_cards[3][0] == self.sorted_cards[4][0]):
      return 'D' + self.sorted_cards[0][0] + self.sorted_cards[3][0]
    return None
  ...
  def flush(self):
    for i in xrange(0, 4):
      if self.cards[i][1] != self.cards[i + 1][1]:
        return None
    return 'E' + ''.join([c[0] for c in self.sorted_cards])
      
    flag = True  
    for i in xrange(1, 4):
      if self.sorted_cards[i][1] != self.sorted_cards[i + 1][1]:
        flag = False
        break
    if flag == True:
      return 'E' + self.sorted_cards[0][0]
    return None
    
  def straight(self):
    order = 'AKQJT98765432'
    if ''.join([card[0] for card in self.sorted_cards]) in order:
      return 'F' + self.sorted_cards[0][0]
    return None
  ...
  def three_of_a_kind(self):
    ...
    for i in xrange(0, 2):
      if self.sorted_cards[i][0] != self.sorted_cards[i + 1][0]:
        ...
        break
    if flag == True:
      return ('G' + self.sorted_cards[0][0] + 
          self.sorted_cards[3][0] + self.sorted_cards[4][0])
    ...
    for i in xrange(1, 3):
      if self.sorted_cards[i][0] != self.sorted_cards[i + 1][0]:
        ...
        break
    if flag == True:
      return ('G' + self.sorted_cards[1][0] + 
          self.sorted_cards[0][0] + self.sorted_cards[4][0])
    ...
    for i in xrange(2, 4):
      if self.sorted_cards[i][0] != self.sorted_cards[i + 1][0]:
        ...
        break
    if flag == True:
      return ('G' + self.sorted_cards[2][0] + 
          self.sorted_cards[0][0] + self.sorted_cards[1][0])
    
    return None
    
  def two_pairs(self):
    if (self.sorted_cards[0][0] == self.sorted_cards[1][0] and
        self.sorted_cards[2][0] == self.sorted_cards[3][0]):
      return ('H' + self.sorted_cards[0][0] + self.sorted_cards[2][0] + 
          self.sorted_cards[4][0])
    if (self.sorted_cards[0][0] == self.sorted_cards[1][0] and
        self.sorted_cards[3][0] == self.sorted_cards[4][0]):
      return ('H' + self.sorted_cards[0][0] + self.sorted_cards[3][0] + 
          self.sorted_cards[2][0])
    if (self.sorted_cards[1][0] == self.sorted_cards[2][0] and
        self.sorted_cards[3][0] == self.sorted_cards[4][0]):
      return ('H' + self.sorted_cards[1][0] + self.sorted_cards[3][0] + 
          self.sorted_cards[0][0])
    return None
    
  def one_pair(self):
    if self.sorted_cards[0][0] == self.sorted_cards[1][0]:
      return ('I' + self.sorted_cards[0][0] + self.sorted_cards[2][0] + 
          self.sorted_cards[3][0] + self.sorted_cards[4][0])
    if self.sorted_cards[1][0] == self.sorted_cards[2][0]:
      return ('I' + self.sorted_cards[1][0] + self.sorted_cards[0][0] + 
          self.sorted_cards[3][0] + self.sorted_cards[4][0])
    if self.sorted_cards[2][0] == self.sorted_cards[3][0]:
      return ('I' + self.sorted_cards[2][0] + self.sorted_cards[0][0] + 
          self.sorted_cards[1][0] + self.sorted_cards[4][0])
    if self.sorted_cards[3][0] == self.sorted_cards[4][0]:
      return ('I' + self.sorted_cards[3][0] + self.sorted_cards[0][0] + 
          self.sorted_cards[1][0] + self.sorted_cards[2][0])
    return None
    
  def high_card(self):
    return 'J' + ''.join([card[0] for card in self.sorted_cards])

def compare(a, b):
  if a[0] != b[0]:
    return a[0] > b[0]
  for i in xrange(1, len(a) - 1):
    if get_key(a[i]) < get_key(b[i]):
      return True
    elif get_key(a[i]) > get_key(b[i]):
      return False
  print 'Haha'
...
sum = 0
for line in input:
  hand_a = PokerHand(line[0:14])
  hand_b = PokerHand(line[15:])
  if not compare(hand_a.get_value(), hand_b.get_value()):
    sum += 1
  # if hand_a.get_value()[0] == hand_b.get_value()[0]:
  # print 'Analysing...'
  # print hand_a.sorted_cards, hand_a.get_value()
  # print hand_b.sorted_cards, hand_b.get_value() 
  # print not compare(hand_a.get_value(), hand_b.get_value())
  # raw_input()
